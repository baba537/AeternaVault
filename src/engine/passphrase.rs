//! How good a passphrase is. Two views are combined:
//!
//! * The usual public guidance (for example the German BSI): long enough,
//!   several kinds of characters, no keyboard rows, sequences, dates or
//!   personal details. These criteria are shown to the user.
//! * An estimate of how many guesses an attacker needs (`zxcvbn`, the method
//!   of Dropbox's strength meter). It sees through repetitions ("123123123…"),
//!   common words, names and leetspeak that simple rules miss.
//!
...
pub struct Assessment {
    ...
    pub personal: bool,
    /// zxcvbn score, 0 (guessed at once) … 4 (very hard to guess).
    pub score: u8,
    /// Estimated number of guesses, as a power of ten.
...
/// zxcvbn looks at this many characters at most (longer is always better).
const MAX_CHECKED: usize = 100;
...
impl Assessment {
    ...
    pub fn rating(&self) -> Rating {
        let long = self.length >= FAIR_LENGTH;
        let varied = self.classes() >= 3 || self.length >= LONG_LENGTH;
        let pattern = self.has_pattern();
        match self.score {
            4 if long => Rating::Good,
            3 if long && varied && !pattern => Rating::Good,
            // Short and built on a word, date or keyboard row: weak however
            // many kinds of characters it has ("Sommer2024!").
            3 | 4 if long || !pattern => Rating::Fair,
            2 if long && !pattern => Rating::Fair,
            _ => Rating::Weak,
        }
    }
}
...
pub fn assess(passphrase: &str, personal: &[&str]) -> Assessment {
    ...
    let checked: String = passphrase.chars().take(MAX_CHECKED).collect();
    let estimate = (!checked.is_empty()).then(|| zxcvbn::zxcvbn(&checked, personal));
    Assessment {
        score: estimate.as_ref().map_or(0, |e| u8::from(e.score())),
        guesses_log10: estimate.as_ref().map_or(0.0, |e| e.guesses_log10()),
...
mod tests {
    ...
    fn short_or_simple_passphrases_are_weak() {
        ...
        assert_eq!(rate("Anna1987Anna1987"), Rating::Weak);
    }

    #[test]
    fn repetitions_are_seen_through() {
        assert_eq!(rate(&"123".repeat(12)), Rating::Weak);
        assert_eq!(rate(&"abc!".repeat(6)), Rating::Weak);
        assert_eq!(rate("Passwort!Passwort!"), Rating::Weak);
    }

    #[test]
    fn long_random_or_many_word_passphrases_are_good() {
        assert_eq!(rate("Kx9!mPv2#qLr7wTe"), Rating::Good);
        assert_eq!(rate("rabe ofen wolke tinte leise"), Rating::Good);
        // Strong but short: only fair.
        assert_eq!(rate("Kx9!mPv2#q"), Rating::Fair);
    }
