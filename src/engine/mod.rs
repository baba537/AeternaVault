...
/// Windows paths are case-insensitive; compare component by component.
pub fn paths_equal(a: &Path, b: &Path) -> bool {
    normalized_components(a) == normalized_components(b)
}

/// `true` if `path` equals `base` or lies inside it (case-insensitive).
...
mod unit_tests {
    ...
    /// The preview must be strictly read-only. The planning modules may not
