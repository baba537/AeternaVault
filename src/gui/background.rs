...
use crate::platform::{self, autostart, file_association};
...
impl AeternaApp {
    ...
    pub fn keeps_running_when_closed(&self) -> bool {
        // Without a notification area icon (Linux), a hidden window could not
        // be reached again, so closing quits.
        self.background.as_ref().is_some_and(|b| b.tray.is_some())
            && self.config.background.keep_running
    }

    /// Whether AeternaVault can wait in the notification area on this system.
    ...
    pub(super) fn take_pending_folders(&mut self, ctx: &egui::Context) {
        ...
        if !self.is_busy() && self.pending.is_none() {
            self.navigate(ctx, super::View::Backup);
        }
    }
    ...
    /// Linux: runs due jobs every five minutes through a systemd user timer,
    /// also while the window is closed.
    pub fn set_systemd_timer(&mut self, enabled: bool) {
        let result = if enabled {
            crate::platform::cli_executable()
                .and_then(|cli| crate::platform::systemd::install(&cli, 5))
                .map(|_| ())
        } else {
            crate::platform::systemd::remove()
        };
        match result {
            Ok(()) => self.record(crate::history::Event::StartWithWindows { on: enabled }),
            Err(err) => {
                tracing::warn!("systemd timer could not be changed: {err}");
                self.notify(
                    NoticeKind::Error,
                    self.lang.autostart_error(&err.to_string()),
                );
